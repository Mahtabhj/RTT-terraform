...
resource "aws_api_gateway_rest_api" "rtt_api_gateway" {
  name        = "rtt-api-gateway"
  description = "rtt API Gateway"
}

resource "aws_api_gateway_resource" "resource_lambda_1" {
  rest_api_id = aws_api_gateway_rest_api.rtt_api_gateway.id
  path_part   = "lambda1"
  parent_id   = aws_api_gateway_rest_api.rtt_api_gateway.root_resource_id
}
...
resource "aws_api_gateway_integration" "integration_alb" {
  uri                     = aws_lb.rtt_alb.dns_name
  type                    = "HTTP_PROXY"
  rest_api_id             = aws_api_gateway_rest_api.rtt_api_gateway.id
  resource_id             = aws_api_gateway_rest_api.rtt_api_gateway.root_resource_id
